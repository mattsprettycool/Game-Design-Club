...
public class MovementCS : MonoBehaviour {
	...
	void Start () {
		...
		BoxCollider2D[] redsBox= new BoxCollider2D[rX];
		foreach (GameObject obj in reds) {
			redsBox[rX-1] = obj.GetComponent<BoxCollider2D> ();
		}
		...
		BoxCollider2D[] bluesBox=new BoxCollider2D[bX];
		foreach (GameObject obj in blues) {
			bluesBox[bX-1] = obj.GetComponent<BoxCollider2D> ();
		}
